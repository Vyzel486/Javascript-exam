...
// 1.
const filterDogOwners = (array) => array.filter(user => user.hasDog).map(person => person.name);
console.log(filterDogOwners(users));

// 2.
// const filterAdults = (array) => array.filter(user => user.age >= 18).map(person => person.name);
const filterAdults = (array) => { 
  const adult = array.filter(person => person.age >= 18); 
  return adult; 
} 
